...
# 优化：去重 Markdown 内容
def deduplicate_markdown(pages: list[str]) -> str:
    deduped = []
    seen = set()
...
def estimate_max_tokens(input_path: Path) -> int:
    try:
        if input_path.suffix.lower() in [".pdf"]:
            ...
            doc.close()
            buffer = 1.2
            return min(int(total_tokens * buffer), 32768)
...
def pdf_to_jpeg(pdf_path: Path, output_jpeg: Path, page_num: int = 0) -> bool:
    try:
        ...
        ) or "Math" in page.get_text()
        zoom = 1.0 if has_complex_content else 1.2
        ...
        if text_blocks:
            ...
            if crop_area < 0.2 * orig_area:  # 提高阈值至 20%
                _log.warning(f"页面 {page_num} 裁剪面积过小 ({crop_area}/{orig_area})，尝试更大区域")
                crop_box = (
                    max(0, int(x0 - margin * 2)),
                    max(0, int(y0 - margin * 2)),
                    min(img.width, int(x1 + margin * 2)),
                    min(img.height, int(y1 + margin * 2))
                )
                crop_area = (crop_box[2] - crop_box[0]) * (crop_box[3] - crop_box[1])
                if crop_area < 0.1 * orig_area:
                    _log.warning(f"裁剪仍过小，回退到无裁剪")
...
def normalize_markdown_headers(markdown: str) -> str:
    ...
    figure_count = 0
    for line in lines:
        if line.startswith("![Figure") or "Figure" in line:
            match = re.match(r"!\[([^\]]*)\]\(([^)]+)\)", line)
            if match:
                alt_text = match.group(1)
                figure_count += 1
                normalized_lines.append(f"**{alt_text}**")
                continue
...
# 优化：增强 OCR 预处理，限制签名区域
def extract_image_text(img: Image.Image) -> Optional[str]:
    try:
        img = ImageEnhance.Contrast(img).enhance(2.0)
        width, height = img.size
        signature_area = img.crop((0, int(height * 0.8), width, height))  # 限制底部 20%
        text = pytesseract.image_to_string(
            signature_area, lang="chi_sim+eng", config="--psm 6 --oem 1 -c force_full_page_ocr=1"
        )
        _log.debug(f"OCR 签名区域文本：{text[:100]}...")
        if text.strip() and len(text.strip()) < 50 and not text.strip().isprintable():
            return text.strip()
        return None
    except Exception as e:
...
def get_adaptive_prompt(input_path: Path) -> str:
    if input_path.suffix.lower() in [".xlsx", ".xls"]:
        ...
        return "将图像转换为 Markdown，提取文本和手写签名（若存在，格式为 <签名: XXX>），保留表格结构，输出纯 Markdown 内容，无额外说明。"
    else:  # PDF
        doc = fitz.open(input_path)
        page_count = len(doc)
        has_images = any(len(page.get_images()) > 0 for page in doc)
        is_academic = any("abstract" in page.get_text().lower() for page in doc)
        doc.close()
        if page_count > 50:
            return "将 PDF 当前页面转换为 Markdown，保留标题、表格和正文，分段清晰，忽略页眉页脚，输出纯 Markdown 内容，无额外说明。"
        elif has_images:
            if is_academic:
                return "将 PDF 当前页面转换为 Markdown，保留文本、表格和图像内容，忽略手写签名，输出纯 Markdown 内容，无额外说明。"
            return "将 PDF 当前页面转换为 Markdown，保留文本、表格和图像内容，手写签名（非印刷体文本，格式为 <签名: XXX>），若无手写签名则不生成签名标记，输出纯 Markdown 内容，无额外说明。"
        return "将 PDF 当前页面转换为 Markdown，保留所有文本、表格和结构，输出纯 Markdown 内容，无额外说明。"
...
async def process_pdf_page(session: aiohttp.ClientSession, input_path: Path, page_num: int, options: ApiVlmOptions, cache_path: Path) -> Optional[str]:
    ...
    try:
        ...
        if markdown_content:
            ...
            markdown_content = re.sub(r"<签名:\s*无>", "", markdown_content)  # 移除 <签名: 无>
            markdown_content = "\n".join(
                line for line in markdown_content.split("\n")
                if not any(keyword in line for keyword in ["以下是", "注：", "表格内容", "图像内容", "如需后续"])
            ).strip()
...
async def process_pdf_async(input_path: Path, pipeline_options: VlmPipelineOptions, cache_path: Path) -> str:
    ...
    return normalize_markdown_headers(deduplicate_markdown(markdown_content))

def estimate_page_tokens(page: fitz.Page) -> int:
    text = page.get_text()
    images = page.get_images()
    return min((len(text) // 3) + (len(images) * 500), 32768)
...
def main():
    ...
    pipeline_options = VlmPipelineOptions(
        ...
        table_structure_options=PdfPipelineOptions().table_structure_options,  # 继承默认设置
    )
    options, _ = blendapi_vlm_options(
        model=model,
        prompt=prompt,
